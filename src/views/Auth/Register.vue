<template>
  <div class="register-root" @mousemove="onMouseMove">
    <!-- 顶部导航栏 -->
    <nav class="top-nav">
      <div class="nav-inner">
        <div class="nav-left">
    ...
    <div class="register-page">
      ...
      <el-card class="register-card">
        <h2 class="register-title">
          <span class="title-char" v-for="(ch, idx) in '用户注册'" :key="idx" :style="{ animationDelay: idx * 0.08 + 's' }">{{ ch }}</span>
        </h2>
...
.register-root {
  ...
  overflow: hidden;
  background: linear-gradient(135deg, #f0e6ff 0%, #e8f4fd 25%, #f0e6ff 50%, #d4eafe 75%, #f0e6ff 100%);
...
.top-nav {
  height: 64px;
  background: linear-gradient(90deg, #722ed1 0%, #9254de 40%, #b37feb 100%);
...
.dot {
  ...
  height: var(--size);
  border-radius: 50%;
  background: rgba(80,30,130,var(--opacity));
  box-shadow: 0 0 4px rgba(120,50,180,calc(var(--opacity) * 0.4));
...
@keyframes eyeGlow {
  0%, 100% { box-shadow: 0 0 12px rgba(167,139,250,0.7), 0 0 24px rgba(167,139,250,0.35), 0 0 48px rgba(139,92,246,0.2); }
  50% { box-shadow: 0 0 18px rgba(167,139,250,0.9), 0 0 36px rgba(167,139,250,0.5), 0 0 60px rgba(139,92,246,0.3); }
}
...
.register-card {
  ...
  box-shadow: 0 8px 32px rgba(114,46,209,0.12), 0 2px 8px rgba(0,0,0,0.06), inset 0 0 0 1px rgba(255,255,255,0.5) !important;
...
.form-input :deep(.el-input__wrapper:hover) {
  box-shadow: 0 4px 16px rgba(114,46,209,0.12) !important;
  transform: translateY(-1px);
}

.form-input :deep(.el-input__wrapper.is-focus) {
  box-shadow: 0 0 0 3px rgba(114,46,209,0.15), 0 4px 16px rgba(114,46,209,0.12) !important;
...
.register-btn:hover {
  ...
  box-shadow: 0 8px 24px rgba(114,46,209,0.4) !important;
}

.register-btn:active { transform: translateY(0) scale(0.98) !important; }
...
@media (max-width: 900px) {
  ...
  .register-card { margin: 0 !important; width: 92%; max-width: 400px; }
}
</style>
